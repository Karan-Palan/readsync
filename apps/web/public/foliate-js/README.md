...
```html
<script type="module">
  import { makeBook, View } from 'https://unpkg.com/foliate-js@latest/dist/index.js';
</script>
```

### Direct Usage

You can also use the modules directly without bundling:

```javascript
import { makeBook, View } from 'foliate-js';
...
<body>
    <input type="file" id="fileInput" accept=".epub,.pdf,.mobi,.fb2,.cbz">
    <div id="viewer"></div>
...
import { makeBook, EPUB } from 'foliate-js';
...
async function analyzeBook(filePath) {
    ...
    if (book instanceof EPUB) {
        const metadata = await book.getMetadata();
        console.log('Title:', metadata.title);
        console.log('Authors:', metadata.creator?.join(', '));
...
- `makeBook(file)` - Create a book instance from a file
- `View` - Main viewer component
- `EPUB`, `MOBI` - Format-specific book classes
- `Paginator`, `FixedLayout` - Layout renderers
- `TTS` - Text-to-speech functionality
- `CFI` - EPUB CFI utilities
