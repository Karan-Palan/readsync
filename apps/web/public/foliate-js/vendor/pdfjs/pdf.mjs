...
class util_FeatureTest {
	static get isLittleEndian() {
		return shadow(util_FeatureTest, "isLittleEndian", isLittleEndian());
	}
	static get isEvalSupported() {
		return shadow(util_FeatureTest, "isEvalSupported", isEvalSupported());
	}
	...
	static get platform() {
		if (
			...
			return shadow(util_FeatureTest, "platform", {
...
class PDFDateString {
	...
	static toDateObject(input) {
		...
		PDFDateString.#regex ||= new RegExp(
			"^D:" +
				"(\\d{4})" +
				"(\\d{2})?" +
				"(\\d{2})?" +
		...
		const matches = PDFDateString.#regex.exec(input);
		if (!matches) {
			return null;
		}
...
function getXfaPageViewport(xfaPage, { scale = 1, rotation = 0 }) {
	...
	const viewBox = [0, 0, Number.parseInt(width), Number.parseInt(height)];
	return new PageViewport({
		viewBox,
		userUnit: 1,
		scale,
...
class ImageManager {
	...
	static get _isSVGFittingCanvas() {
		...
		return shadow(ImageManager, "_isSVGFittingCanvas", promise);
	}
...
class AnnotationEditor {
	...
	static get _defaultLineColor() {
		return shadow(
			AnnotationEditor,
			"_defaultLineColor",
			AnnotationEditor._colorManager.getHexCode("CanvasText"),
		);
	}
	...
	static initialize(l10n, _uiManager) {
		...
		AnnotationEditor._borderLineWidth =
			Number.parseFloat(style.getPropertyValue("--outline-width")) || 0;
	}
	...
	fixDims() {
		...
		const [parentWidth, parentHeight] = this.parentDimensions;
		if (!widthPercent) {
			style.width = `${((100 * Number.parseFloat(width)) / parentWidth).toFixed(2)}%`;
		}
		if (!this.#keepAspectRatio && !heightPercent) {
			style.height = `${((100 * Number.parseFloat(height)) / parentHeight).toFixed(2)}%`;
		}
	}
	...
	static async deserialize(data, parent, uiManager) {
		const editor = new AnnotationEditor.prototype.constructor({
			parent,
			id: parent.getNextId(),
			uiManager,
		});
...
class AnnotationStorage {
	...
	get serializable() {
		...
		for (const [key, val] of this.#storage) {
			const serialized =
				val instanceof AnnotationEditor ? val.serialize(false, context) : val;
			if (serialized) {
...
class GlobalWorkerOptions {
	...
	static get workerPort() {
		return GlobalWorkerOptions.#port;
	}
	static set workerPort(val) {
		...
		GlobalWorkerOptions.#port = val;
	}
	static get workerSrc() {
		return GlobalWorkerOptions.#src;
	}
	static set workerSrc(val) {
		if (typeof val !== "string") {
			throw new Error("Invalid `workerSrc` type.");
		}
		GlobalWorkerOptions.#src = val;
	}
		...
		return value.replaceAll(
			...
			(matches, charset, encoding, text) => {
				if (encoding === "q" || encoding === "Q") {
					...
					text = text.replaceAll(/=([0-9a-fA-F]{2})/g, (match, hex) =>
...
function validateRangeRequestCapabilities({
	...
	const length = Number.parseInt(responseHeaders.get("Content-Length"), 10);
		...
		if (xhrStatus === PARTIAL_CONTENT_RESPONSE) {
			...
			if (matches) {
				pendingRequest.onDone({
					begin: Number.parseInt(matches[1], 10),
					chunk,
				});
			} else {
...
class TextLayer {
	...
	static cleanup() {
		if (TextLayer.#pendingTextLayers.size > 0) {
			return;
		}
		TextLayer.#ascentCache.clear();
		for (const { canvas } of TextLayer.#canvasContexts.values()) {
			canvas.remove();
		}
		TextLayer.#canvasContexts.clear();
	}
	static #getCtx(lang = null) {
		let ctx = TextLayer.#canvasContexts.get((lang ||= ""));
		if (!ctx) {
			...
			TextLayer.#canvasContexts.set(lang, ctx);
			TextLayer.#canvasCtxFonts.set(ctx, {
				size: 0,
				family: "",
			});
		}
	...
	static #ensureCtxFont(ctx, size, family) {
		const cached = TextLayer.#canvasCtxFonts.get(ctx);
	...
	static #ensureMinFontSizeComputed() {
		...
		TextLayer.#minFontSize = div.getBoundingClientRect().height;
		div.remove();
	}
	static #getAscent(fontFamily, lang) {
		const cachedAscent = TextLayer.#ascentCache.get(fontFamily);
		if (cachedAscent) {
			return cachedAscent;
		}
		const ctx = TextLayer.#getCtx(lang);
		ctx.canvas.width = ctx.canvas.height = DEFAULT_FONT_SIZE;
		TextLayer.#ensureCtxFont(ctx, DEFAULT_FONT_SIZE, fontFamily);
		const metrics = ctx.measureText("");
		let ascent = metrics.fontBoundingBoxAscent;
		let descent = Math.abs(metrics.fontBoundingBoxDescent);
		if (ascent) {
			const ratio = ascent / (ascent + descent);
			TextLayer.#ascentCache.set(fontFamily, ratio);
...
const DefaultCMapReaderFactory = isNodeJS
...
function getDocument(src = {}) {
	...
	const isImageDecoderSupported =
		typeof src.isImageDecoderSupported === "boolean"
			...
			: !isNodeJS &&
				(util_FeatureTest.platform.isFirefox || !globalThis.chrome);
...
class PDFWorker {
	...
	static fromPort(params) {
		...
		const cachedPort = PDFWorker.#workerPorts?.get(params.port);
		if (cachedPort) {
	...
	static get _setupFakeWorkerGlobal() {
		const loader = async () => {
			if (PDFWorker.#mainThreadWorkerMessageHandler) {
				return PDFWorker.#mainThreadWorkerMessageHandler;
			}
			const worker = await import(/*webpackIgnore: true*/ PDFWorker.workerSrc);
			return worker.WorkerMessageHandler;
		};
		return shadow(PDFWorker, "_setupFakeWorkerGlobal", loader());
	}
}
...
const build = "f9bea397f"; // ./src/shared/scripting_utils.js
...
class ColorConverters {
	...
	static CMYK_HTML(components) {
		const rgb = ColorConverters.CMYK_RGB(components).slice(1);
		return ColorConverters.RGB_HTML(rgb);
	}
...
class XfaLayer {
	...
	static setAttributes({ html, element, storage = null, intent, linkService }) {
		...
		if (storage && attributes.dataId) {
			XfaLayer.setupStorage(html, attributes.dataId, element, storage);
		}
	}
	...
	#show() {
		...
		if (!this.isVisible) {
			...
			this.#container.style.zIndex =
				Number.parseInt(this.#container.style.zIndex) + 1000;
		} else if (this.#pinned) {
...
class FreeTextEditor extends AnnotationEditor {
	...
	static initialize(l10n, uiManager) {
		...
		const style = getComputedStyle(document.documentElement);
		FreeTextEditor._internalPadding = Number.parseFloat(
	...
	static async deserialize(data, parent, uiManager) {
		...
		const editor = await AnnotationEditor.deserialize(data, parent, uiManager);
...
class FreeDrawOutliner {
	...
	constructor({ x, y }, box, scaleFactor, thickness, isLTR, innerMargin = 0) {
		...
		this.#isLTR = isLTR;
		this.#last.set([Number.NaN, Number.NaN, Number.NaN, Number.NaN, x, y], 6);
	...
	getOutlines() {
		...
		for (let i = 0; i < N; i += 2) {
			if (isNaN(top[i])) {
				outline[i] = outline[i + 1] = Number.NaN;
				continue;
			}
			outline[i] = top[i];
			outline[i + 1] = top[i + 1];
		...
		for (let i = bottom.length - 6; i >= 6; i -= 6) {
			for (let j = 0; j < 6; j += 2) {
				if (isNaN(bottom[i + j])) {
					outline[N] = outline[N + 1] = Number.NaN;
					N += 2;
					continue;
				}
	...
	#getOutlineStart(outline, pos) {
		const bottom = this.#bottom;
		outline.set(
			[Number.NaN, Number.NaN, Number.NaN, Number.NaN, bottom[4], bottom[5]],
			pos,
		);
		return (pos += 6);
	}
...
class FreeDrawOutline extends Outline {
	...
	constructor(outline, points, box, scaleFactor, innerMargin, isLTR) {
		...
		this.#innerMargin = innerMargin;
		this.#isLTR = isLTR;
		this.lastPoint = [Number.NaN, Number.NaN];
...
class HighlightEditor extends AnnotationEditor {
	...
	static startHighlighting(parent, isLTR, { target: textLayer, x, y }) {
		...
		const pointerUpCallback = (e) => {
			ac.abort();
			HighlightEditor.#endHighlight(parent, e);
		};
		window.addEventListener("blur", pointerUpCallback, {
			signal,
		});
		...
		textLayer.addEventListener(
			"pointermove",
			HighlightEditor.#highlightMove.bind(HighlightEditor, parent),
			{
				signal,
			},
		);
		HighlightEditor._freeHighlight = new FreeHighlightOutliner(
			{
				x,
				y,
			},
			...
			parent.scale,
			HighlightEditor._defaultThickness / 2,
			isLTR,
			0.001,
		);
		({
			id: HighlightEditor._freeHighlightId,
			clipPathId: HighlightEditor._freeHighlightClipId,
		} = parent.drawLayer.draw(
			{
				bbox: [0, 0, 1, 1],
				root: {
					viewBox: "0 0 1 1",
					fill: HighlightEditor._defaultColor,
					"fill-opacity": HighlightEditor._defaultOpacity,
				},
				rootClass: {
					highlight: true,
					free: true,
				...
				path: {
					d: HighlightEditor._freeHighlight.toSVGPath(),
				},
			},
			true,
			true,
	...
	static #highlightMove(parent, event) {
		if (HighlightEditor._freeHighlight.add(event)) {
			parent.drawLayer.updateProperties(HighlightEditor._freeHighlightId, {
				path: {
					d: HighlightEditor._freeHighlight.toSVGPath(),
				},
			});
	...
	static #endHighlight(parent, event) {
		if (!HighlightEditor._freeHighlight.isEmpty()) {
			parent.createAndAddNewEditor(event, false, {
				highlightId: HighlightEditor._freeHighlightId,
				highlightOutlines: HighlightEditor._freeHighlight.getOutlines(),
				clipPathId: HighlightEditor._freeHighlightClipId,
				methodOfCreation: "main_toolbar",
			});
		} else {
			parent.drawLayer.remove(HighlightEditor._freeHighlightId);
		}
...
class DrawingEditor extends AnnotationEditor {
	...
	static #currentPointerId = Number.NaN;
	static #currentPointerType = null;
	static #currentPointerIds = null;
	static #currentMoveTimestamp = Number.NaN;
	...
	static updateDefaultParams(type, value) {
		const propertyName = DrawingEditor.typesMap.get(type);
		if (propertyName) {
			DrawingEditor._defaultDrawingOptions.updateProperty(propertyName, value);
		}
		if (DrawingEditor._currentParent) {
			DrawingEditor.#currentDraw.updateProperty(propertyName, value);
			DrawingEditor._currentParent.drawLayer.updateProperties(
				DrawingEditor._currentDrawId,
				DrawingEditor._defaultDrawingOptions.toSVGProperties(),
			);
		}
	}
	updateParams(type, value) {
	...
	static get defaultPropertiesToUpdate() {
		const properties = [];
		const options = DrawingEditor._defaultDrawingOptions;
		for (const [type, name] of DrawingEditor.typesMap) {
	...
	static startDrawing(parent, uiManager, _isLTR, event) {
		...
		window.addEventListener(
			...
			(e) => {
				if (DrawingEditor.#currentPointerId === e.pointerId) {
					DrawingEditor._currentParent.endDrawingSession();
				} else {
					DrawingEditor.#currentPointerIds?.delete(e.pointerId);
				}
			},
			...
			(e) => {
				...
				if (DrawingEditor.#currentDraw.isCancellable()) {
					...
					if (DrawingEditor.#currentDraw.isEmpty()) {
						DrawingEditor._currentParent.endDrawingSession(true);
					} else {
						DrawingEditor._endDraw(null);
					}
				}
			},
			{
		...
		target.addEventListener(
			"pointermove",
			DrawingEditor._drawMove.bind(DrawingEditor),
			{
				signal,
			},
		);
		...
		uiManager.updateUIForDefaultProperties(DrawingEditor);
		DrawingEditor.#currentDraw = DrawingEditor.createDrawerInstance(
			x,
			y,
			parentWidth,
			parentHeight,
		...
		DrawingEditor.#currentDrawingOptions =
			DrawingEditor.getDefaultDrawingOptions();
		DrawingEditor._currentParent = parent;
		({ id: DrawingEditor._currentDrawId } = parent.drawLayer.draw(
			DrawingEditor._mergeSVGProperties(
	...
	static _cleanup(all) {
		if (all) {
			DrawingEditor._currentDrawId = -1;
			DrawingEditor._currentParent = null;
			DrawingEditor.#currentDraw = null;
			DrawingEditor.#currentDrawingOptions = null;
			DrawingEditor.#currentPointerType = null;
			DrawingEditor.#currentMoveTimestamp = Number.NaN;
		}
		if (DrawingEditor.#currentDrawingAC) {
			DrawingEditor.#currentDrawingAC.abort();
			DrawingEditor.#currentDrawingAC = null;
			DrawingEditor.#currentPointerId = Number.NaN;
			DrawingEditor.#currentPointerIds = null;
		}
	}
	static _endDraw(event) {
		const parent = DrawingEditor._currentParent;
		if (!parent) {
			return;
		}
		parent.toggleDrawing(true);
		DrawingEditor._cleanup(false);
		if (event) {
			parent.drawLayer.updateProperties(
				DrawingEditor._currentDrawId,
		...
		if (DrawingEditor.supportMultipleDrawings) {
			const draw = DrawingEditor.#currentDraw;
			const drawId = DrawingEditor._currentDrawId;
		...
		DrawingEditor.endDrawing(false);
	}
	static endDrawing(isAborted) {
		...
		if (!DrawingEditor.#currentDraw.isEmpty()) {
			...
			const editor = parent.createAndAddNewEditor(
				...
				false,
				{
					drawId: DrawingEditor._currentDrawId,
					drawOutlines: DrawingEditor.#currentDraw.getOutlines(
						pageWidth * scale,
						pageHeight * scale,
						scale,
						DrawingEditor._INNER_MARGIN,
					),
					drawingOptions: DrawingEditor.#currentDrawingOptions,
					mustBeCommitted: !isAborted,
				},
			);
			DrawingEditor._cleanup(true);
			return editor;
		}
		parent.drawLayer.remove(DrawingEditor._currentDrawId);
		DrawingEditor._cleanup(true);
		return null;
	}
	...
	static async deserialize(data, parent, uiManager) {
		...
		const drawOutlines = DrawingEditor.deserializeDraw(
			pageX,
			pageY,
			pageWidth,
			pageHeight,
...
class InkDrawOutliner {
	...
	add(x, y) {
		...
		if (isNaN(x1)) {
			...
			this.#line.push(Number.NaN, Number.NaN, Number.NaN, Number.NaN, x, y);
			return {
				path: {
					d: this.toSVGPath(),
				},
	...
	static deserialize(
		...
		if (!lines) {
			...
			for (const point of points) {
				...
				line.set([Number.NaN, Number.NaN, Number.NaN, Number.NaN, x1, y1], 0);
...
class InkEditor extends DrawingEditor {
	...
	static getDefaultDrawingOptions(options) {
		const clone = InkEditor._defaultDrawingOptions.clone();
	...
	static createDrawerInstance(x, y, parentWidth, parentHeight, rotation) {
		return new InkDrawOutliner(
			...
			parentHeight,
			rotation,
			InkEditor._defaultDrawingOptions["stroke-width"],
		);
	}
	static deserializeDraw(
		pageX,
	...
	static async deserialize(data, parent, uiManager) {
		...
		const editor = await DrawingEditor.deserialize(data, parent, uiManager);
		editor.annotationElementId = data.id || null;
		editor._initialData = initialData;
		return editor;
	}
	...
	static onScaleChangingWhenDrawing() {
		const parent = InkEditor._currentParent;
		if (!parent) {
			return;
		}
		DrawingEditor.onScaleChangingWhenDrawing();
		InkEditor._defaultDrawingOptions.updateSVGProperty("stroke-width");
		parent.drawLayer.updateProperties(
			InkEditor._currentDrawId,
			InkEditor._defaultDrawingOptions.toSVGProperties(),
		);
	}
...
class StampEditor extends AnnotationEditor {
	...
	static get supportedTypesStr() {
		return shadow(
			StampEditor,
			"supportedTypesStr",
			StampEditor.supportedTypes.join(","),
		);
	}
	static isHandlingMimeForPasting(mime) {
		return StampEditor.supportedTypes.includes(mime);
	}
	static paste(item, parent) {
...
class DrawLayer {
	...
	static get _svgFactory() {
		return shadow(DrawLayer, "_svgFactory", new DOMSVGFactory());
	}
