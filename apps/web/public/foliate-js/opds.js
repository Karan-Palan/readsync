...
export const getFeed = (doc) => {
	...
	for (const entry of entries) {
		...
		const groupLinks =
			linksByRel.get(REL.GROUP) ?? linksByRel.get("collection");
		const groupLink = groupLinks?.length
			? (groupLinks.find((link) => groupedItems.has(link.href)) ??
				groupLinks[0])
			: null;
		...
		const item = isPub
			...
			: Object.assign(
					links.find((link) => isOPDSCatalog(link.type)) ?? links[0] ?? {},
					{
						title: children.find(filter("title"))?.textContent,
...
export const getOpenSearch = (doc) => {
	...
	const $url =
		$$urls.find((url) => isOPDSCatalog(url.getAttribute("type"))) ?? $$urls[0];
	...
	return {
		metadata: {
			title: (
				children.find(filter("LongName")) ?? children.find(filter("ShortName"))
			)?.textContent,
		...
		search: (map) =>
			template.replace(regex, (_, prefix, param) => {
				...
				return encodeURIComponent(
					val ? val : !ns ? (defaultMap.get(param) ?? "") : "",
				);
			}),
		params: Array.from(
			template.matchAll(regex),
			([, prefix, param, optional]) => {
				const namespace = prefix ? $url.lookupNamespaceURI(prefix) : null;
				const ns = namespace === defaultNS ? null : namespace;
				return {
					ns,
